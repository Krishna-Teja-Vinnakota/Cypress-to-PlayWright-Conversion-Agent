...
// Playwright test for Profile Form validation - 10 Essential Tests
...
test.describe('Profile Form Validation', () => {
  ...
  test('Test 8: Should enable submit button and submit successfully with all valid data', async ({ page }) => {
    ...
    // Verify submission
    await page.waitForResponse(response => response.url().includes('/api/profile') && response.request().method() === 'PUT');
